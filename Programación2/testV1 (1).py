...
class MainWindow(QMainWindow):
    ...
    def mostrarPanelMedico(self, id_medico):
        ...
        font = QFont("Times New Roman", 18)
         ...
         # 🔍 Obtener nombre del médico desde la lista
        doctores_raw = client.getDoctorsList("", "")
        print("Respuesta cruda del servidor:", doctores_raw)
        doctores = json.loads(doctores_raw)
        print("Decodificado:", doctores)


        if isinstance(doctores, str):
    # Caso de error: el servidor devolvió un mensaje
            QMessageBox.warning(self, "Error", doctores)
            nombre_medico = "Nombre no disponible"  # "Identificación no encontrada"
            
            print("Buscando médico con id:", id_medico)
            
        else:
    # Caso normal: doctores es lista o dict
            nombre_medico = next((d["name"] for d in doctores if str(d.get("identificacion")) == str(id_medico)),"Nombre no encontrado")


        
        label = QLabel(f"Bienvenido, Dr. {nombre_medico}")
        label.setFont(font)
        ...
        elif isinstance(citas_decodificadas, list):
            citas = citas_decodificadas
        else:
            citas = []
            QMessageBox.warning(self, "Error", citas_decodificadas)
