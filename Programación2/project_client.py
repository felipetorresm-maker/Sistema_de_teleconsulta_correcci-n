...
class Client:
    ...
    def getDoctorsList(self, id="", name=""):    
        response = requests.get(self.url + '/listdoctors', params={'id': id, 'name': name})
        return response.content.decode('utf-8')
    
    # Función la IP del médicot
    def getDoctorIP(self, id, name):    
        response = requests.get(self.url + '/getdoctorip', params={'id': id, 'name': name})
        return response.content.decode('utf-8')
